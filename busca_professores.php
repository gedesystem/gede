...
<section>
    ...
    if ($opcao == "id_inep") {
        $sql_seleciona = "SELECT matricula_uefs, cpf, nome, id_gede, id_inep FROM docentes_dados_cadastrais WHERE id_inep LIKE '%$chave%'";
        $resultado = seleciona($sql_seleciona);
    } else if ($opcao == "nome") {
        $sql_seleciona = "SELECT matricula_uefs, cpf, nome, id_gede, id_inep  FROM docentes_dados_cadastrais WHERE nome LIKE '%$chave%'";
        $resultado = seleciona($sql_seleciona);
    } else if ($opcao == "cpf") {
        $sql_seleciona = "SELECT matricula_uefs, cpf, nome, id_gede, id_inep  FROM docentes_dados_cadastrais WHERE cpf LIKE '%$chave%'";
        $resultado = seleciona($sql_seleciona);
    } else if ($opcao == "matricula_uefs") {
        $sql_seleciona = "SELECT matricula_uefs, cpf, nome, id_gede, id_inep  FROM docentes_dados_cadastrais WHERE matricula_uefs LIKE '%$chave%'";
        $resultado = seleciona($sql_seleciona);
    } else if ($opcao == "codigo_departamento") {
        $sql_seleciona = "SELECT matricula_uefs, cpf, nome, id_gede, id_inep  FROM docentes_dados_cadastrais WHERE codigo_departamento LIKE '%$chave%'";
        $resultado = seleciona($sql_seleciona);
    }
    ?>
